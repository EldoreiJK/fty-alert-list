/*  =========================================================================
    fty_alert_list_selftest.c - run selftests
...
// Tests for stable public classes:
    /*{ "alert", alert_test, true, true, NULL },
    { "fty_alert_actions", fty_alert_actions_test, true, true, NULL },
    { "alert_list", alert_list_test, true, true, NULL },*/
...
{
    test_item_t *item;
    printf ("Running fty-alert-list selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_ALERT_LIST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_alert_list_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_alert_list_selftest.c [options] ...");
    ...
    if (test) {
        ...
        if (!test->subtest)
            ...
            fty_alert_list_private_selftest (verbose, test->subtest);
#endif // FTY_ALERT_LIST_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
